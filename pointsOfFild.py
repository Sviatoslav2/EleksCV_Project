...
def get_pred_mask(point_glob_pix, imgMatrixRead, model, sample_size, present=False):
    if present:
        return get_origin_mask(imgMatrixRead, imgMatrixRead.mask_path, point_glob_pix, sample_size)
    else:
        return postprocess_modul.postproces(postprocess_modul.get_prediction(model, point_sumple.get_sumple_of_point(imgMatrixRead.lst_path,point_glob_pix,imgMatrixRead, sample_size)))
...
def point_of_fild(mask, point_glob_pix, imgMatrixRead, imgcord, sumple_size, array_res):
    ...
    def add_point_to_stack(point, array_res, mask, stack):
        if (not mask[point[0]][point[1]]) and (not array_res[point[0]][point[1]]):
